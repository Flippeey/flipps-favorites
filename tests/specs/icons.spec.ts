...
test('Firefox uses background.html, not a service worker (MV3 CORS regression check)', async ({
  bgPage,
}, testInfo) => {
  test.skip(testInfo.project.name !== 'firefox', 'Firefox-only test');
  // If this assertion fails it means Firefox is no longer using background.html,
  // which was the root cause of the CORS errors in the icon service worker.
  expect(bgPage.url()).toContain('background.html');
});
